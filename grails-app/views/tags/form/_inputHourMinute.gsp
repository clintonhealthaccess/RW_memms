<div class="row ${hasErrors(bean:bean,field:field,'errors')}">
	...
	<div  class="period-month-year">
		<input type="text" class="idle-field numbers-only ${dateClass}" id="${name +'_hours'}" maxlength="${maxlength}" name="${name +'_hours'}" value="${hours}" ${active} ${readonly?'readonly="readonly"':''}/>
		<label for="${name +'_hours'}">${message(code:'entity.hours.label')}</label>
	</div>
	<div class="period-month-year">
		<input type="text" class="idle-field numbers-only ${dateClass}" id="${name +'_minutes'}" maxlength="${maxlength}" name="${name +'_minutes'}" value="${minutes}" ${active} ${readonly?'readonly="readonly"':''}/>
