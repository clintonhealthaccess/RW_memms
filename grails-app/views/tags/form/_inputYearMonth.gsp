<div class="row ${hasErrors(bean:bean,field:field,'errors')}">
	...
	<div  class="period-month-year">
		<input type="text" class="idle-field numbersOnly ${dateClass}" id="${name +'_years'}" maxlength="${maxlength}" name="${name +'_years'}" value="${years}" ${active} ${readonly?'readonly="readonly"':''}/>
		<label for="${name +'_years'}">${message(code:'entity.years.label')}</label>
	</div>
	<div class="period-month-year">
		<input type="text" class="idle-field numbersOnly ${dateClass}" id="${name +'_months'}" maxlength="${maxlength}" name="${name +'_months'}" value="${months}" ${active} ${readonly?'readonly="readonly"':''}/>
