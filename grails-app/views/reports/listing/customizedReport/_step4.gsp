...
<div class="dialog-form step-4" id='js-step-4'>
  <!-- Step 4 -->
  <h2>${message(code:'listing.report.name.selection.label')}Select name for customized report<span class="right"><g:message code="reports.step" args="['4','4']"/></span></h2>
  ...
  <g:form name="formStep4Next" url="[action:'customizedListing', params: step4Params]" update="dialog-form">
    <fieldset>
      <ul>
        <li>
          <label for="customizedReportName">${message(code:'listing.report.name.label')}</label>
          <input name="customizedReportName" type="text" />
        </li>
        <li>
          <label for="customizedReportSave">${message(code:'listing.report.save.label')}</label>
          <input name="customizedReportSave" type="checkbox" />
        </li>
      </ul>
    </fieldset>
  </g:form>
  <g:formRemote name="formRemoteStep4Prev" url="[action:'step3', params:step4Params]" update="dialog-form"
