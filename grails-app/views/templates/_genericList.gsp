	     	...
	     	<g:if test="${!search}">
		     	...
		     	<shiro:hasPermission permission="${controllerName}:create">
					<g:if test="${!addTemplate}">
		  				<a href="${createLinkWithTargetURI(controller: (controller)?controller:controllerName, action:'create', params:['location.id': location?.id,'dataLocation.id': dataLocation?.id,'equipment.id':equipment?.id,'workOrder.id':workOrder?.id,'preventiveOrder.id':preventiveOrder?.id])}" class="next medium left push-r">
